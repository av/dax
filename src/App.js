...
function SkippedOpenFolder() {
    ...
    return (_jsx("button", { onClick: () => void openFolder(), disabled: isLoading, style: {
            padding: '12px 32px',
            fontSize: '16px',
        ...
        }, children: isLoading ? 'Opening\u2026' : 'Open a Folder' }));
}
function App() {
    ...
    useEffect(() => {
        if (hasShownFirstActionToast)
            return;
        const doneCount = agentSteps.filter((s) => s.status === 'done').length;
        if (doneCount > prevDoneCountRef.current && doneCount >= 1) {
            useToastStore.getState().addToast('You can approve, edit, or reject any step.', 'info', 5000);
    ...
    if (!rootPath && isOnboarding && currentStep === 'folderSelect') {
        return (_jsxs(_Fragment, { children: [_jsx(Onboarding, {}), _jsx(ToastContainer, {})] }));
    }
    if (!rootPath && !isOnboarding) {
        ...
        return (_jsxs("div", { style: {
                ...
                gap: '24px',
                        ...
                        margin: 0,
                        color: '#7aa2f7',
                    }, children: "Dax" }), _jsx(SkippedOpenFolder, {}), _jsx(ToastContainer, {})] }));
    }
    // Folder is open — render 3D workspace with HUD overlay
    return (_jsx(ErrorBoundary, { children: _jsxs("div", { style: { position: 'relative', width: '100vw', height: '100vh' }, onContextMenu: (e) => e.preventDefault(), children: [_jsx(Workspace, {}), _jsx(HUD, {}), isOnboarding && !hasCompletedOnboarding && _jsx(Onboarding, {}), _jsx(ToastContainer, {})] }) }));
}
export default App;
