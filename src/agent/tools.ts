...
export function setToolLLMConfig(config: LLMConfig): void {
  _llmConfig = config;
}
...
const searchFilesTool: Tool = {
  ...
  execute: async (args) => {
    ...
    const lines = matches.map(
      (n) => `${n.type === 'directory' ? '📁' : '📄'} ${n.path} (${(n.sizeBytes / 1024).toFixed(1)} KB)`,
    );
    ...
    const messages: LLMMessage[] = [
      {
        role: 'system',
        content:
          'You are a project organization expert. Analyze the file tree and suggest 3-5 concrete actions that would improve the project structure. Be specific — refer to actual files and directories.',
      },
