...
function createParticles(): TrailParticle[] {
...
export default function AgentEntity() {
  ...
  const trailPositions = useMemo(
    () => new Float32Array(TRAIL_PARTICLE_COUNT * 3),
    [],
  );
  const trailSizes = useMemo(
    () => new Float32Array(TRAIL_PARTICLE_COUNT),
    [],
  );
  ...
  const targetPosRef = useRef<Vec3Tuple | null>(null);
  ...
  useFrame((_state, delta) => {
    ...
    // Trail color
    (trailMat.uniforms.uColor.value as THREE.Color).set(color);
    ...
    const trailGeo = (trailMeshRef.current as unknown as THREE.Points)?.geometry;
    if (trailGeo) {
      const posAttr = trailGeo.getAttribute('position');
      if (posAttr) {
        (posAttr as THREE.BufferAttribute).set(trailPositions);
        posAttr.needsUpdate = true;
      }
      const sizeAttr = trailGeo.getAttribute('aSize');
      if (sizeAttr) {
        (sizeAttr as THREE.BufferAttribute).set(trailSizes);
        sizeAttr.needsUpdate = true;
      }
      const opAttr = trailGeo.getAttribute('aOpacity');
      if (opAttr) {
        (opAttr as THREE.BufferAttribute).set(trailOpacities);
