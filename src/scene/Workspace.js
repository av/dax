 ...
 */
function getDepth(dirPath, rootPath) {
...
export default function Workspace() {
    ...
    const visibleFiles = useMemo(() => {
        if (visibleFileCount >= files.length)
            return files;
        return files.slice(0, visibleFileCount);
    }, [files, visibleFileCount]);
    return (_jsxs(Canvas, { camera: { position: [0, 15, 25], fov: 55, near: 0.1, far: 500 }, style: { width: '100vw', height: '100vh', background: '#07070d' }, gl: { antialias: true, alpha: false }, dpr: [1, 2], children: [_jsx("ambientLight", { intensity: 0.4 }), _jsx("directionalLight", { position: [20, 30, 10], intensity: 0.8, castShadow: false }), _jsx("directionalLight", { position: [-10, 20, -10], intensity: 0.3 }), _jsx("fog", { attach: "fog", args: ['#07070d', 50, 250] }), _jsx("hemisphereLight", { args: ['#1a1a3e', '#07070d', 0.4] }), _jsx("pointLight", { position: [-20, 40, -20], intensity: 0.15, color: "#6366f1", decay: 2 }), _jsx(CameraController, {}), mergedLayoutMap.has('__root__') && (_jsx(DirectoryPlatform, { name: rootPath?.split('/').pop() ?? 'root', entry: mergedLayoutMap.get('__root__'), depth: 0 })), directories.map((dir) => {
                const entry = mergedLayoutMap.get(dir.id);
                if (!entry || !entry.platformSize)
                    return null;
                return (_jsx(DirectoryPlatform, { name: dir.name, entry: entry, depth: getDepth(dir.path, rootPath ?? '') }, dir.id));
            }), visibleFiles.length > 0 && (_jsx(FileInstances, { files: visibleFiles, layoutMap: mergedLayoutMap })), _jsx(AgentEntity, {}), _jsx(SelectionBox, { layoutMap: mergedLayoutMap }), _jsx(FileDragger, { layoutMap: mergedLayoutMap }), import.meta.env.DEV && _jsx(PerformanceMonitor, {}), _jsx(AdaptiveDpr, { pixelated: true }), _jsx(Preload, { all: true })] }));
}
